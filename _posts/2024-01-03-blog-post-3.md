...
$$ a\cup (b \cup c) = a \cub b \cup c $$, and later goes to define another
...
In section 33 of page 54 he also introduces the a "dot product" by means of an 
argument on how if you were to have two extensive distances
$B_0$ and $B_1$ and were to use a "comparison operator" $B_0 \cdot B_1$, that'd be zero,
but otherwise it is commutative.

Less appreciated but still remarkable is the introduction of a more general geometric
product on page 61:
...
Grassman comes to define "*synthetic connections*":
